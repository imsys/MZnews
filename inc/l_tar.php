...
/*************************************************************
|
|   Retirado e traduzido do código fonte do
|   ========================================
...
|
| $tar = new tar();
| $tar->new_tar("/pasta", "meuTar.tar");
| $files = $tar->list_files();
| $tar->extract_files( "/extrair/para/este/diretorio" );
|
| USO DA CRIAÇÃO:
|
| $tar = new tar();
| $tar->new_tar("/pasta" , "meuNovoTar.tar");
| $tar->current_dir("/dir" );  // Opcional - fala ao script qual diretório
|                                            você está para sincronizar a
|                                            criação dos arquivos
| $tar->add_files( $array_dos_nomes_dos_arquivos_com_path );
| (ou $tar->add_directory( "/pasta/meuDiretorio" ); para arquivar um diretório inteiro)
| $tar->write_tar();
|
*************************************************************/
...
class tar {
	...
	//+--------------------------------------------------------------------------
	// CONSTRUTOR: Tenta adivinhar o diretório atual.
	//+--------------------------------------------------------------------------
	...
	function tar() {
		...
		// Define alguns atributos, eles poderão ser alterados futuramente
	...
	//+--------------------------------------------------------------------------
	// Define o tarname. Se estamos extraindo um tarball, então deve ser um path
	// para o tarball, e o seu nome (ex: $tar->new_tar("/dir/pasta" ,'meuTar.tar')
	// ou se estamos criando um tar, ele deve ser o caminho e o nome do arquivo tar
	// a ser criado.
	//+--------------------------------------------------------------------------
	...
	function new_tar($tarpath, $tarname) {
		...
		// Cerficar que não há uma barra no final do caminho
	...
	//+--------------------------------------------------------------------------
	// Maneira fácil de alterar os padrões
	//+--------------------------------------------------------------------------
	...
	//+--------------------------------------------------------------------------
	// Usuário determina o diretório raíz para a criação/extração do tarball
	//+--------------------------------------------------------------------------
	...
	//+--------------------------------------------------------------------------
	// listar arquivos: retorna uma array com todos os nomes dos arquivos do tar
	//+--------------------------------------------------------------------------
	
	function list_files($advanced="") {
	
		// $advanced == "" - retorna apenas o nome
		// $advanced == 1  - retorna o nome, tamanho, mtime e mode
